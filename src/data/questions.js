const questions = {
  ...
  "Depth First Search": [{ "": { "choicesWithFeedback": { choice1: "feedback1", choice2: "feedback2", choice3: "feedback2", choice4: "feedback2" } } }, {}],
  Queues: [{}, {}],
  Stacks: [{}, {}],
  "Big O Notation": [
    {
      "Order these from worst to best:": {
        "choicesWithFeedback": {
          "O(n!) < O(2^n) < O(n^2) < O(n log n) < O(n) < O(log n) < O(1)" : "You got it! ",
          "O(1) < O(log n) < O(n) < O(n log n) < O(n^2) < O(2^n) < O(n!)" : "Order from worst to best not best to worst, try again",
          "O(n!) < O(n^2) < O(2^n) < O(n log n) < O(n) < O(log n) < O(1)" : "Close, try again",
          "O(1) < O(log n) < O(n log n) < O(n) < O(n^2) < O(2^n) < O(n!)" : "Try again"
        }
      }
    }, {}],
  BUD: [{}, {}],
